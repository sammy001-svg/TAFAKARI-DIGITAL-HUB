    ...
    try {
        $stmt = db()->prepare(
            "SELECT p.*, u.name AS authorName, u.username AS authorUsername
             ...
             WHERE p.id = ? AND p.type = 'VIDEO' AND p.status = 'PUBLISHED' LIMIT 1"
        );
        $stmt->execute([$id]);
        $post = $stmt->fetch();
        if ($post) {
...
if (!$post) {
    ...
    echo '<main class="flex-grow flex flex-col items-center justify-center py-24 text-center px-6">
            ...
            <h1 class="font-outfit text-4xl font-black text-slate-900 mb-4">Video Not Found</h1>
            <p class="text-slate-500 mb-8">This video may have been removed or is not yet published.</p>
            <a href="/videos" class="inline-block px-6 py-3 rounded-xl font-bold text-sm" style="background:#E7952A;color:#0D0102">&larr; Back to Videos</a>
          </main>';
...
try {
    $stmt = db()->prepare(
        "SELECT id, title, thumbnailUrl, country, issueCategory, viewCount, createdAt
         FROM Post WHERE type='VIDEO' AND status='PUBLISHED' AND id != ?
...
$embedHtml     = !empty($mediaUrl) ? video_embed_html($mediaUrl, $post['title']) : '';
$hasEmbed      = $embedHtml !== '';
$authorDisplay = $post['authorName'] ?? $post['authorUsername'] ?? 'Staff';

$protocol    = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
$shareUrl    = $protocol . '://' . ($_SERVER['HTTP_HOST'] ?? '') . '/videos/' . rawurlencode($id);
...
<main class="flex-grow">
  ...
  <!-- ── Video hero ─────────────────────────────────────────────────────── -->
  <div style="background:#0D0102" class="py-10 px-6">
    <div class="max-w-5xl mx-auto">
      <nav class="flex items-center gap-2 text-xs text-white/40 mb-5">
        <a href="/videos" class="hover:text-amber-400 transition-colors font-bold text-white/60">Videos</a>
        <span>&rsaquo;</span>
      ...
      <div class="flex flex-wrap gap-2 mb-4">
        ...
        <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-white/10 text-white/60">Video</span>
      </div>
      <h1 class="font-outfit text-3xl md:text-4xl font-black text-white leading-tight mb-3"><?= h($post['title']) ?></h1>
      <div class="flex flex-wrap items-center gap-4 text-xs text-white/40">
        ...
        <?php if (($post['viewCount'] ?? 0) > 0): ?><span>&bull; <?= format_number((int)$post['viewCount']) ?> views</span><?php endif; ?>
      </div>
    </div>
  </div>
  ...
  <div class="max-w-5xl mx-auto px-6 py-10">
    ...
    <!-- Video player -->
    <?php if ($hasEmbed): ?>
      <div class="rounded-3xl overflow-hidden shadow-xl border border-amber-100 mb-10" style="aspect-ratio:16/9;background:#0D0102">
        <?= $embedHtml ?>
      </div>
    <?php elseif (!empty($post['thumbnailUrl'])): ?>
      <div class="w-full rounded-3xl overflow-hidden shadow-md border border-amber-100 mb-10" style="max-height:500px">
    ...
    <?php if (!empty($post['description'])): ?>
      <div class="bg-white rounded-2xl border border-amber-100 p-6 md:p-8 shadow-sm mb-8">
        <p class="text-slate-600 text-[15px] leading-relaxed"><?= h($post['description']) ?></p>
      </div>
    <?php endif; ?>
    ...
    <?php if (!$hasEmbed && !empty($mediaUrl)): ?>
      <div class="my-8 p-6 rounded-2xl border border-amber-200" style="background:#F8F8F0">
        <p class="text-sm font-bold text-amber-900 mb-3">Video Link</p>
        <a href="<?= h($mediaUrl) ?>" target="_blank" rel="noopener noreferrer"
           class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all hover:brightness-110"
    ...
    <?php if (!empty($related)): ?>
      <section class="mt-16 pt-12 border-t border-slate-100">
        <h2 class="font-outfit font-bold text-xl text-slate-900 mb-6">More Videos</h2>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <?php foreach ($related as $r): ?>
            <a href="/videos/<?= h($r['id']) ?>" class="group flex flex-col bg-white rounded-2xl overflow-hidden border border-amber-100 hover:shadow-md hover:-translate-y-1 transition-all">
              <div class="relative h-36 overflow-hidden" style="background:#0D0102">
                <?php if (!empty($r['thumbnailUrl'])): ?>
                  <img src="<?= h($r['thumbnailUrl']) ?>" alt="" class="w-full h-full object-cover opacity-75 group-hover:scale-105 transition-transform duration-500">
                <?php endif; ?>
                <div class="absolute inset-0 flex items-center justify-center" style="background:rgba(0,0,0,.35)">
                  <div class="w-10 h-10 rounded-full flex items-center justify-center" style="background:rgba(117,11,37,.85)">
                    <svg width="16" height="16" fill="white" viewBox="0 0 24 24" style="margin-left:2px"><path d="M8 5v14l11-7z"/></svg>
                  </div>
                </div>
              </div>
              <div class="p-4">
