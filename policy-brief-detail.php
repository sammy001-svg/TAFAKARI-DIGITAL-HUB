    ...
    try {
        $stmt = db()->prepare(
            "SELECT p.*, u.name AS authorName, u.username AS authorUsername
             ...
             WHERE p.id = ? AND p.type = 'POLICY_BRIEF' AND p.status = 'PUBLISHED' LIMIT 1"
        );
        $stmt->execute([$id]);
        $post = $stmt->fetch();
        if ($post) {
...
if (!$post) {
    ...
    echo '<main class="grow flex flex-col items-center justify-center py-24 text-center px-6">
            ...
            <h1 class="font-outfit text-4xl font-black text-slate-900 mb-4">Policy Brief Not Found</h1>
            <p class="text-slate-500 mb-8">This policy brief may have been removed or is not yet published.</p>
            <a href="/policy-briefs" class="inline-block px-6 py-3 rounded-xl font-bold text-sm" style="background:#E7952A;color:#0D0102">&larr; Back to Policy Briefs</a>
          </main>';
...
try {
    $stmt = db()->prepare(
        "SELECT id, title, thumbnailUrl, country, issueCategory, mediaUrl, createdAt
         FROM Post WHERE type='POLICY_BRIEF' AND status='PUBLISHED' AND id != ?
...
$shareUrl    = $protocol . '://' . ($_SERVER['HTTP_HOST'] ?? '') . '/policy-briefs/' . rawurlencode($id);
$downloadUrl = '/policy-briefs/' . rawurlencode($id) . '?download=1';
...
<main class="grow">
  ...
  <div style="background:#0D0102" class="py-12 px-6">
    <div class="max-w-4xl mx-auto">
      <nav class="flex items-center gap-2 text-xs text-white/40 mb-6">
        <a href="/policy-briefs" class="hover:text-amber-400 transition-colors font-bold text-white/60">Policy Briefs</a>
        <span>&rsaquo;</span>
  ...
  <div class="max-w-4xl mx-auto px-6 py-12">
    ...
    <?php if (!empty($mediaUrl)): ?>
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6 bg-white rounded-3xl border border-indigo-100 shadow-md p-6 md:p-8 mb-10">
        ...
        <div class="flex-1 min-w-0">
          <p class="font-outfit font-bold text-lg text-slate-900 mb-1 leading-snug">Download the Full Brief</p>
    ...
    <?php if (!empty($post['content'])): ?>
      <h2 class="font-outfit font-bold text-xl text-slate-900 mb-4">Full Policy Brief</h2>
    ...
    <section class="mt-12">
      ...
      <?php elseif (!$commentSuccess): ?>
        <div class="text-center py-12 bg-white rounded-2xl border border-amber-100">
          <p class="text-slate-400 text-sm">No comments yet. Be the first to respond to this policy brief!</p>
        </div>
      <?php endif; ?>
    </section>
    ...
    <?php if (!empty($related)): ?>
      <section class="mt-16 pt-12 border-t border-slate-100">
        <h2 class="font-outfit font-bold text-xl text-slate-900 mb-6">Related Policy Briefs</h2>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <?php foreach ($related as $r): ?>
            <a href="/policy-briefs/<?= h($r['id']) ?>" class="group flex flex-col bg-white rounded-2xl overflow-hidden border border-amber-100 hover:shadow-md hover:-translate-y-1 transition-all">
              <div class="h-28 overflow-hidden" style="background:#0D0102">
                <?php if (!empty($r['thumbnailUrl'])): ?>
                  <img src="<?= h($r['thumbnailUrl']) ?>" alt="<?= h($r['title']) ?>" class="w-full h-full object-cover">
                <?php endif; ?>
              </div>
              <div class="p-4">
