...
$w      = $isSuper ? '1' : 'authorId = ?';
$params = $isSuper ? []  : [$uid];

$run = function (string $sql) use ($pdo, $params) {
    $st = $pdo->prepare($sql);
    $st->execute($params);
    return $st;
};

$totalPosts     = (int)$run("SELECT COUNT(*) FROM Post WHERE $w")->fetchColumn();
$publishedPosts = (int)$run("SELECT COUNT(*) FROM Post WHERE $w AND status='PUBLISHED'")->fetchColumn();
$pendingPosts   = (int)$run("SELECT COUNT(*) FROM Post WHERE $w AND status='PENDING'")->fetchColumn();
$draftPosts     = (int)$run("SELECT COUNT(*) FROM Post WHERE $w AND status='DRAFT'")->fetchColumn();
$rejectedPosts  = (int)$run("SELECT COUNT(*) FROM Post WHERE $w AND status='REJECTED'")->fetchColumn();
$archivedPosts  = (int)$run("SELECT COUNT(*) FROM Post WHERE $w AND status='ARCHIVED'")->fetchColumn();

$views = $run("SELECT COALESCE(SUM(viewCount),0) AS v, COALESCE(SUM(downloadCount),0) AS d FROM Post WHERE $w")->fetch();
