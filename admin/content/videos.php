...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'create') {
    ...
    elseif (!$c_mediaUrl)      $createError = 'Video URL is required.';
...
$stCounts = $pdo->prepare("SELECT status, COUNT(*) as cnt FROM Post WHERE type='VIDEO'" . (!$isSuper ? " AND authorId=?" : "") . " GROUP BY status");
$stCounts->execute(!$isSuper ? [$uid] : []);
$statusCounts = ['ALL' => $total];
foreach ($stCounts->fetchAll() as $row) $statusCounts[$row['status']] = (int)$row['cnt'];

function vidHref(string $status, int $p, string $search = ''): string {
    $q = [];
    if ($status !== 'ALL') $q['status'] = $status;
    if ($p > 1)            $q['page']   = $p;
    if ($search !== '')    $q['q']      = $search;
    return '/admin/content/videos' . ($q ? '?' . http_build_query($q) : '');
}

$pageTitle      = 'Videos | Tafakari Admin';
$adminPageTitle = 'Videos';
$adminPageSub   = $total . ' video' . ($total !== 1 ? 's' : '') . ($statusFilter !== 'ALL' ? ' · ' . strtolower($statusFilter) : '');
?>
...
<main class="flex-1 overflow-y-auto p-6 md:p-8">
  ...
  <div class="flex items-center justify-between mb-6">
    <div>
      <div class="flex items-center gap-2 mb-1">
        ...
        <span class="text-[11px] font-bold text-slate-600">Videos</span>
      </div>
      <h1 class="font-outfit font-black text-2xl text-slate-900">Video Library</h1>
    </div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <?php
    $stats = [
      ['label'=>'Videos',    'val'=>$total,                           'color'=>'#750B25'],
  ...
  <!-- Search -->
  <form method="GET" action="/admin/content/videos" class="flex flex-wrap items-center gap-3 mb-5">
    <div class="relative flex-grow max-w-sm">
      ...
      <input type="text" name="q" value="<?= h($search) ?>" placeholder="Search videos…"
    ...
    <?php if ($search !== ''): ?>
      <a href="<?= h(vidHref($statusFilter, 1)) ?>" class="px-4 py-2.5 rounded-xl text-[12px] font-bold border border-slate-200 text-slate-600 hover:bg-slate-50 bg-white shadow-sm">Clear</a>
    <?php endif; ?>
  </form>
  ...
  <?php if (empty($posts)): ?>
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 text-center">
      ...
      <p class="text-slate-500 mt-2 text-sm mb-6"><?= $search !== '' ? 'No results for "' . h($search) . '".' : 'Add your first video to the library.' ?></p>
      <button type="button" onclick="openCreateDrawer()" class="btn-primary" style="padding:.65rem 1.5rem;border:none;cursor:pointer">Add Video</button>
    </div>
  <?php else: ?>
    <!-- Video card grid -->
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow group">
        ...
        <div class="relative aspect-video bg-slate-900 overflow-hidden">
          <?php if (!empty($p['thumbnailUrl'])): ?>
            <img src="<?= h($p['thumbnailUrl']) ?>" alt="" class="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity" loading="lazy" onerror="this.style.display='none'">
          <?php endif; ?>
          <!-- Play overlay -->
          <div class="absolute inset-0 flex items-center justify-center">
            <?php if ($hasVideo): ?>
              <div class="w-12 h-12 rounded-full bg-white/90 flex items-center justify-center shadow-lg">
                <svg width="16" height="16" fill="#750B25" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
              </div>
            <?php else: ?>
              <div class="px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-wider" style="background:rgba(239,68,68,.9);color:#fff">No Video URL</div>
            <?php endif; ?>
          </div>
          <!-- Status badge -->
          <div class="absolute top-2 left-2"><?= status_badge($p['status']) ?></div>
          <!-- Views -->
          <?php if (($p['viewCount'] ?? 0) > 0): ?>
            <div class="absolute bottom-2 right-2 px-2 py-1 rounded-lg text-[10px] font-bold text-white" style="background:rgba(0,0,0,.6)">
              <?= number_format((int)$p['viewCount']) ?> views
            </div>
          <?php endif; ?>
        </div>
        <!-- Info -->
        <div class="p-4">
          <p class="text-[14px] font-semibold text-slate-900 leading-snug line-clamp-2 mb-1"><?= h($p['title']) ?></p>
          <div class="flex items-center gap-2 text-[11px] text-slate-400 mb-3">
            <span><?= h($p['country']) ?></span>
            <span>&bull;</span>
            <span><?= format_relative_time($p['updatedAt']) ?></span>
            <?php if ($isSuper): ?><span>&bull;</span><span><?= h($p['authorName'] ?? '—') ?></span><?php endif; ?>
          </div>
          <div class="flex items-center gap-1.5">
            ...
            <?php endif; ?>
            <?php if ($canDelete): ?>
              <button onclick="postAction('/api/posts/<?= h($p['id']) ?>','DELETE',this,'Delete this video?')"
                      class="px-3 py-1.5 text-[10px] font-bold rounded-lg bg-rose-100 text-rose-700 hover:bg-rose-200 transition-colors">Del</button>
            <?php endif; ?>
          </div>
        </div>
      </div>
    ...
    <?php if ($totalPages > 1): ?>
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <?php if ($page > 1): ?>
            <a href="<?= h(vidHref($statusFilter, $page-1, $search)) ?>" class="px-3 py-2 text-[11px] font-bold bg-white rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50">&larr; Prev</a>
          <?php else: ?>
          ...
          <?php if ($page < $totalPages): ?>
            <a href="<?= h(vidHref($statusFilter, $page+1, $search)) ?>" class="px-3 py-2 text-[11px] font-bold bg-white rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50">Next &rarr;</a>
          <?php else: ?>
...
<div id="create-drawer"
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-bottom:1px solid #f1f5f9;flex-shrink:0;position:sticky;top:0;background:#fff;z-index:1">
    <div>
      <p style="font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.14em;color:#750B25;margin:0 0 3px">Add Video</p>
  ...
  <form method="POST" action="/admin/content/videos" onsubmit="return uwCheckRequired(this)" style="flex:1;padding:24px;display:flex;flex-direction:column;gap:18px">
    <input type="hidden" name="_action" value="create">

    <div>
      <label style="display:block;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.12em;color:#94a3b8;margin-bottom:7px">Title *</label>
    ...
    <?php uw_img('vid-thumb','thumbnailUrl','Thumbnail','Preview image shown before playback',($createError ? ($_POST['thumbnailUrl'] ?? '') : '')); ?>
    <?php uw_media('vid-media','mediaUrl','Video URL *','video','video/mp4,video/webm,video/ogg','MP4, WebM · max 500 MB','YouTube / Vimeo link, or direct video URL','https://www.youtube.com/watch?v=…',($createError ? ($_POST['mediaUrl'] ?? '') : ''),true); ?>

    <div>
      <label style="display:block;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.12em;color:#94a3b8;margin-bottom:7px">Video Description</label>
      <textarea name="content" rows="5"
                ...
                placeholder="Full description or transcript…"><?= h(($createError ? $_POST['content'] : '') ?? '') ?></textarea>
    </div>
