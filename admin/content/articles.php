...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'create') {
    ...
    elseif (!$c_issueCategory) $createError = 'Issue category is required.';
        ...
        )->execute([$id,$c_title,'ARTICLE',$c_description,$c_content,$c_thumbnailUrl,'',$c_country,$c_region,$c_issueCategory,$status,$uid]);
        header('Location: /admin/content/articles?created=' . ($status === 'PUBLISHED' ? 'pub' : '1'));
        exit;
    }
}
...
<main class="flex-1 overflow-y-auto p-6 md:p-8">
  ...
  <?php if (empty($posts)): ?>
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 text-center">
      ...
      <button type="button" onclick="openCreateDrawer()" class="btn-primary" style="padding:.65rem 1.5rem;border:none;cursor:pointer">Write an Article</button>
    </div>
  <?php else: ?>
...
<div id="create-drawer"
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-bottom:1px solid #f1f5f9;flex-shrink:0;position:sticky;top:0;background:#fff;z-index:1">
    <div>
      <p style="font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.14em;color:#750B25;margin:0 0 3px">New Article</p>
      <p style="font-size:13px;font-weight:600;color:#0f172a;margin:0">Draft saved automatically on submit</p>
    </div>
  ...
  <form method="POST" action="/admin/content/articles" onsubmit="return uwCheckRequired(this)" style="flex:1;padding:24px;display:flex;flex-direction:column;gap:18px">
    ...
    <div>
      ...
      <textarea name="content" rows="10"
                style="width:100%;padding:11px 14px;border:1.5px solid #e2e8f0;border-radius:12px;background:#f8fafc;font-size:13px;font-family:monospace;box-sizing:border-box;outline:none;resize:vertical"
                placeholder="Write in Markdown…"><?= h($_POST['content'] ?? '') ?></textarea>
    </div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  b.textContent = isPub ? 'Article published successfully.' : 'Article saved as draft.';
