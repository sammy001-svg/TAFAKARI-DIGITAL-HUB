    ...
    try {
        $stmt = db()->prepare(
            "SELECT p.*, u.name AS authorName, u.username AS authorUsername
             ...
             WHERE p.id = ? AND p.type = 'PODCAST' AND p.status = 'PUBLISHED' LIMIT 1"
        );
        $stmt->execute([$id]);
        $post = $stmt->fetch();
        if ($post) {
...
if (!$post) {
    ...
    echo '<main class="flex-grow flex flex-col items-center justify-center py-24 text-center px-6">
            ...
            <a href="/podcasts" class="inline-block px-6 py-3 rounded-xl font-bold text-sm" style="background:#E7952A;color:#0D0102">&larr; Back to Podcasts</a>
          </main>';
...
try {
    $stmt = db()->prepare(
        "SELECT id, title, thumbnailUrl, country, issueCategory, createdAt
         FROM Post WHERE type='PODCAST' AND status='PUBLISHED' AND id != ?
...
$isDirectAudio = !empty($mediaUrl) && is_audio_url($mediaUrl);
$isExtLink     = !empty($mediaUrl) && !$isDirectAudio;
$authorDisplay = $post['authorName'] ?? $post['authorUsername'] ?? 'Staff';

$protocol    = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
$shareUrl    = $protocol . '://' . ($_SERVER['HTTP_HOST'] ?? '') . '/podcasts/' . rawurlencode($id);
...
$pageMediaUrl  = $isDirectAudio ? $mediaUrl : '';
$pageUrl       = $shareUrl;
?>
...
<main class="flex-grow">
  ...
  <div style="background:#0D0102" class="py-12 px-6">
    <div class="max-w-4xl mx-auto">
      <nav class="flex items-center gap-2 text-xs text-white/40 mb-6">
        <a href="/podcasts" class="hover:text-amber-400 transition-colors font-bold text-white/60">Podcasts</a>
        <span>&rsaquo;</span>
      ...
      <div class="flex flex-wrap gap-2 mb-5">
        ...
        <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-white/10 text-white/60">Podcast</span>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-4 text-xs text-white/40">
        ...
        <?php if (($post['viewCount'] ?? 0) > 0): ?><span>&bull; <?= format_number((int)$post['viewCount']) ?> plays</span><?php endif; ?>
      </div>
    </div>
  </div>
  ...
  <div class="max-w-4xl mx-auto px-6 py-12">
    ...
    <!-- Artwork + player card -->
    <div class="bg-white rounded-3xl border border-amber-100 shadow-md overflow-hidden mb-10">
      <div class="flex flex-col md:flex-row">
        <!-- Artwork -->
        <div class="md:w-56 md:shrink-0 h-52 md:h-auto relative" style="background:#0D0102;min-height:14rem">
          <?php if (!empty($post['thumbnailUrl'])): ?>
            <img src="<?= h($post['thumbnailUrl']) ?>" alt="<?= h($post['title']) ?>"
                 class="w-full h-full object-cover opacity-80">
          <?php endif; ?>
          <div class="absolute inset-0 flex items-center justify-center" style="background:rgba(13,1,2,.45)">
            <div class="w-16 h-16 rounded-full flex items-center justify-center" style="background:#E7952A">
        ...
        <div class="flex-1 p-6 md:p-8 flex flex-col justify-center">
          <p class="text-[10px] font-black uppercase tracking-widest mb-3" style="color:#E7952A">Now Playing</p>
          <h2 class="font-outfit font-bold text-xl text-slate-900 mb-4 leading-snug"><?= h($post['title']) ?></h2>
          <?php if ($isDirectAudio): ?>
            <audio controls class="w-full rounded-xl" style="accent-color:#E7952A">
              <source src="<?= h($mediaUrl) ?>">
              Your browser does not support audio playback.
            </audio>
          <?php elseif ($isExtLink): ?>
            <div class="flex flex-wrap gap-3">
              <a href="<?= h($mediaUrl) ?>" target="_blank" rel="noopener noreferrer"
                 class="inline-flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-sm text-white transition-all hover:opacity-90"
          ...
          <?php else: ?>
            <p class="text-sm text-slate-400 italic">Audio not yet available.</p>
          <?php endif; ?>
        </div>
      </div>
    </div>

    <!-- Episode body / show notes -->
    <?php if (!empty($post['content'])): ?>
      <h2 class="font-outfit font-bold text-xl text-slate-900 mb-4">Show Notes</h2>
    ...
    <?php if (!empty($related)): ?>
      <section class="mt-16 pt-12 border-t border-slate-100">
        <h2 class="font-outfit font-bold text-xl text-slate-900 mb-6">More Podcast Episodes</h2>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <?php foreach ($related as $r): ?>
            <a href="/podcasts/<?= h($r['id']) ?>" class="group flex items-center gap-4 bg-white rounded-2xl p-4 border border-amber-100 hover:shadow-md hover:-translate-y-1 transition-all">
              <div class="w-14 h-14 rounded-xl shrink-0 overflow-hidden" style="background:#0D0102">
                <?php if (!empty($r['thumbnailUrl'])): ?>
                  <img src="<?= h($r['thumbnailUrl']) ?>" alt="" class="w-full h-full object-cover opacity-75">
                <?php else: ?>
                  <div class="w-full h-full flex items-center justify-center">
                    <svg width="20" height="20" fill="none" stroke="#E7952A" stroke-width="1.5" viewBox="0 0 24 24"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
                  </div>
                <?php endif; ?>
              </div>
